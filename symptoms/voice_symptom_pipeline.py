...
if not os.getenv("OPENAI_API_KEY"):
    raise ValueError("\u274c OPENAI_API_KEY is not set. Please load your .env file or export the key.")

client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))

audio_dir = "data/patient_conversations"
db_path = "bayman_agentx_health.db"
...
def extract_structured_symptoms(transcript):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a clinical assistant extracting structured symptoms from a doctor-patient conversation."},
            {"role": "user", "content": f'''Given this transcript, extract the symptoms mentioned by the patient.
...
def generate_physician_summary(transcript):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You summarize clinical conversations into physician-friendly notes."},
            {"role": "user", "content": f'''Summarize the following doctor-patient conversation for a physician.
...
def process_audio_to_symptoms_and_summary(audio_path, patient_id="Unknown"):
    ...
    if summary_row and symptom_row:
        print(f"🔄 Skipping processing. Fetching stored results for {audio_filename}...")
...
def store_results_in_sqlite(results, db_path="bayman_agentx_health.db"):
    ...
    try:
        if isinstance(symptoms_json, str):
            ...
            if symptoms_json.startswith("```json"):
                symptoms_json = symptoms_json.replace("```json", "").replace("```", "").strip()
        ...
        secondary_symptoms = [s['symptom'] for s in all_symptoms if s['symptom'] != primary_symptom]
        alert_flag = any(s.lower() in ['chest pain', 'shortness of breath'] for s in [primary_symptom] + secondary_symptoms)
    ...
    for attempt in range(3):
        try:
            ...
            if cursor.fetchone():
                print(f"⚠️ Entry already exists for {patient_id} - {audio_file}. Skipping insert.")
                return
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (
                patient_id, recorded_datetime, audio_file, transcript_text, summary_text
            ))
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                patient_id, recorded_datetime, audio_file, primary_symptom,
                json.dumps(secondary_symptoms), symptom_details, int(alert_flag)
            ))

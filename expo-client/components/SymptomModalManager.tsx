...
export default function SymptomModalManager() {
  ...
  const openEditModal = (index: number) => {
    const sym = symptoms[index];
    const primary = sym.primary_symptoms[0];
    const details = sym.symptom_details[primary];
    setFormData({ ...details });
    ...
    const newSym: Symptom = {
      ...
      symptom_details: {
        [symptomName]: {
          ...
          characteristics: formData.characteristics?.split(',').map((s: string) => s.trim()) ?? [],
          triggers: formData.triggers?.split(',').map((s: string) => s.trim()) ?? [],
          aggravating_factors: formData.aggravating_factors?.split(',').map((s: string) => s.trim()) ?? [],
          relieving_factors: formData.relieving_factors?.split(',').map((s: string) => s.trim()) ?? [],
          associated_symptoms: formData.associated_symptoms?.split(',').map((s: string) => s.trim()) ?? [],
        },
      },
    };
            ...
            return (
              <TouchableOpacity key={index} onPress={() => openEditModal(index)}>
                <View className="p-4 mb-2 bg-white rounded-xl shadow">
                  ...
                  <Text className="text-gray-600">Severity: {detail?.severity}</Text>
                </View>
              </TouchableOpacity>
            );
          })}
